...
TerminaStore: BEGIN
	...
	ManejoErrores:BEGIN
		...
		IF(Par_NumDias < 5) THEN
			IF(Par_SabHab=SabHabSi)THEN
				SELECT DATE_ADD(Par_FechaInicio, INTERVAL Par_NumDias DAY) AS FECHA;
			ELSE 
				SELECT DATE_ADD(Par_FechaInicio, INTERVAL Par_NumDias+1 DAY) AS FECHA;
			END IF
		ELSE
